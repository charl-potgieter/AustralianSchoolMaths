...
class SiteHierarchies:
    ...
    def _first_hierarchy_with_inconsistent_sort_order(self) -> list[str]:
        ...
        for i in range(len(self._hierarchy_data.columns)):
            ...
            ordered_paths = list(current_level_data.apply(list, axis=1))
            inconsistent_item = self._first_non_consecutive_like_item(
                ordered_paths
            )
    ...
    def _first_non_consecutive_like_item(
        self, input_values: list[list[str]]
    ) -> list[str]:
    ...
    def get_sort_index_in_parent_path(self, dir_to_find: str) -> int:
        ...
        """
        dir_paths = dir_to_find.split(os.path.sep)
        unique_truncated_hierarchies = self.truncate_to_path_length(
            len(dir_paths)
        ).unique
        if len(dir_paths) > 1:
            hierarchies_with_same_parent = (
                unique_truncated_hierarchies.filter_by_path_start(
                    dir_paths[:-1]
                )
            )
        else:
    ...
    def filter_by_path_start(self, path_to_filter: list[str]) -> Self:
        ...
        filtered_hierarchies = hierarchies_to_filter[
            hierarchies_to_search.apply(tuple, axis=1) == tuple(path_to_filter)
        ]
    ...
    def directory_paths(self) -> list[str]:
        ...
        paths = self._hierarchy_data.apply(
            lambda x: os.path.sep.join(list(x.dropna()[:-1])), axis="columns"
        )
    ...
    def _directory_paths_as_dataframe(self) -> pd.DataFrame:
        ...
        """
        dir_paths = pd.DataFrame(
            self._hierarchy_data.apply(
                lambda x: x.dropna()[:-1], axis="columns"
            ).drop_duplicates()
        )
        return dir_paths
    ...
    def all_path_levels(self) -> list[str]:
        ...
        for column_index in range(len(dir_paths)):
            paths_at_current_level = dir_paths.iloc[:, : column_index + 1]
            paths_at_current_level = (
                paths_at_current_level.drop_duplicates().dropna()
            )
