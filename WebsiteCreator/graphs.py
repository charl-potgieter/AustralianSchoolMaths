...
class GraphCore():
    ...
    def __init__(self,
                 ...
                 domain_set: sp.Interval = sp.Interval(-10, 10),
                 points_to_plot: int = 1000, fig_width: float = 10,
                 fig_height: float = 10,
                 x_tick_mark_type: str = 'standard',
                 y_tick_mark_type: str = 'standard',
                 graph_buffer_over_domain: float = 0.1,
    ...
    @property
    def _x_limits(self) -> tuple[float]:
        ''' Returns a tuple of min and max value of x allowing for a buffer over the domain'''
        x_range = self._domain_end - self._domain_start
        x_lim_min = (self._domain_start
                     - x_range * self.graph_buffer_over_domain)
        x_lim_max = float(self._domain_end
                          + x_range * self.graph_buffer_over_domain)
        return (x_lim_min, x_lim_max)

    @property
    def _domain_start(self) -> float:
        return cast(self.domain_set.start, float)

    @property
    def _domain_end(self) -> float:
        return cast(self.domain_set.end, float)
...
class graph_straight_line():
    ...
    def __init__(self,
        ...
        if line_is_a_function:
            ...
            self._graph = GraphCore(graph_label=graph_label)
