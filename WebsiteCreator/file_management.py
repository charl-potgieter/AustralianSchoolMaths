...
class _MarkdownContent:
    ...
    def add_front_matter_property(
        self, property_key: str | float, property_value: str | float
    ) -> None:
...
class IndexFiles:
    ...
    def _get_weight_based_on_hierarchies(self, path: str) -> int:
        weight = self._hierarchies.get_sort_index_in_parent_path(path) + 1
        return weight
...
class FormulaFiles:
    def __init__(
        self, base_path: str, hierarchies: SiteHierarchies, formulas: Formulas
    ) -> None:
        self._base_path = base_path
        self._hierarchies = hierarchies
    ...
    def _get_path_in_hierarchy(
        self, formula_table: FormulaTable, is_cumulative_by_year: bool
    ) -> str:
...
class TopicFile:
    ...
    def _formulas_by_subtopic(self, subtopic):
        return self._formulas.filter_by_dict({"Syllabus_subtopic": subtopic})
    ...
    def _add_notes(self, notes_by_subtopic: Notes) -> None:
        ...
        if len(notes_by_subtopic.data):
            ...
            self._markdown_content.add_content(note_to_display)

    def _add_formula_tables(self, formulas_by_subtopic: Formulas) -> None:
        for table_type in formula_table_types:
            formula_table = FormulaTable(formulas_by_subtopic, table_type)
            if formula_table.contains_content:
...
class TopicFiles:
    ...
    def iterate(self):
        for syllabus_topic in self._syllabus.topics():
            ...
            path_in_hierarchy = self._get_path_in_hierarchy(
                syllabus_topic.is_cumulative,
                syllabus_topic.state,
